...
const SavedRecipesPage = () => {
  const { savedRecipes, removeSavedRecipe } = useContext(RecipeContext);
  const { profile } = useContext(AuthContext);


  const handleDoubleClick = async (recipeId) => {
    const success = await removeSavedRecipe(recipeId);
    if (success) {
      toast.success("Recipe removed successfully!");
    } else {
      toast.error("Failed to remove the recipe.");
    }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {savedRecipes.map((recipe) => (
              <motion.div
                ...
                transition={{ duration: 0.4 }}
                onDoubleClick={() => handleDoubleClick(recipe._id)} 
              >
                <img
                  src={recipe.image}
                  alt={recipe.title}
